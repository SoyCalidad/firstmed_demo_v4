    ...
    def write(self, vals):
        ...
        if self.country_id.code == 'PE':
            if self.l10n_latam_identification_type_id.l10n_pe_vat_code == '6':
                ...
                if not self.vat or len(self.vat) != 8:
                    raise UserError(
        ...
        for values in vals:
            if values.get('vat'):
                ...
                if values.get('l10n_latam_identification_type_id'):
                    document_type = self.env['l10n_latam.identification.type'].browse(
                        values['l10n_latam_identification_type_id'])
                    if document_type.l10n_pe_vat_code == '6':
                        if len(values.get('vat')) != 11:
                            raise UserError(
                                _('El RUC debe tener 11 dígitos.'))
                    elif document_type.l10n_pe_vat_code == '1':
                        if len(values.get('vat')) != 8:
                            raise UserError(
                                _('El DNI debe tener 8 dígitos.'))
