...
class AccountMove(models.Model): 
	...
	def _get_l10n_latam_document_type_id(self):
		for move in self:
			if move.move_type and move.journal_id:    
	...
	def _get_invoice_values_odoofact(self):
		...
		values = {
			...
			'descuento_global': abs(self.l10n_pe_edi_global_discount) if self.move_type not in ['in_refund', 'out_refund'] else '',
			'total_descuento': abs(self.l10n_pe_edi_amount_discount) if self.move_type not in ['in_refund', 'out_refund'] else '',
		...
		if self.move_type == "out_refund":
			...
			for line in lines:
				...
				if line.display_type == False:
					print(line.name)
					if "anticipo" in line.name.lower():
						total_anticipo = total_anticipo + abs(line.l10n_pe_edi_amount_free if line.l10n_pe_edi_free_product else line.price_total)
						print("anticipo")
			
			print("move_type")
			print(self.move_type)
			print(total_anticipo)
			...
			for line in lines:
				if line.display_type == False:
					...
					if line.price_subtotal < 0 and not line.l10n_pe_edi_regularization_advance:
						continue
					print("Valores de la rectificativa con anticipo")
					if total_anticipo == 0 or line.l10n_pe_edi_free_product:
						values = {
					...
					elif total_anticipo < abs(line.l10n_pe_edi_amount_free if line.l10n_pe_edi_free_product else line.price_total) and total_anticipo != 0:
						ref_precio_unitario = round(abs((line.l10n_pe_edi_amount_free if line.l10n_pe_edi_free_product else line.price_total) - total_anticipo)/abs(line.quantity), 2)
						ref_total 		    = round(ref_precio_unitario * abs(line.quantity), 2)
						ref_subtotal  	    = round(ref_total/1.18, 2)
						ref_igv		 	    = round(ref_total - ref_subtotal, 2)
						ref_valor_unitario  = round(ref_subtotal/abs(line.quantity), 6)
						_logger.info("############# Valores del Item #############")
						_logger.info("ref_precio_unitario")
						_logger.info( ref_precio_unitario )
						_logger.info("ref_total")
						_logger.info( ref_total )
						_logger.info("ref_subtotal")
						_logger.info( ref_subtotal )
						_logger.info("ref_igv")
						_logger.info( ref_igv )
						_logger.info("ref_valor_unitario")
						_logger.info( ref_valor_unitario )
						_logger.info("")
						values = {
							'unidad_de_medida': line.product_uom_id and 
							...
							'descripcion': line.name,
							'cantidad': abs(line.quantity),
							'valor_unitario': ref_valor_unitario,
							'precio_unitario': ref_precio_unitario,
							'descuento': abs(line.l10n_pe_edi_amount_discount),
					...
					elif total_anticipo >= abs(line.l10n_pe_edi_amount_free if line.l10n_pe_edi_free_product else line.price_total):
						values = {
							'unidad_de_medida': line.product_uom_id and 
												(line.product_uom_id.l10n_pe_edi_uom_code_id and line.product_uom_id.l10n_pe_edi_uom_code_id.code or False) or 
												(line.product_id and (line.product_id.type != 'service' and 'NIU' or 'ZZ') or 'ZZ'),
