...
class AccountMove(models.Model):
	...
	def action_match(self):
		if self.total_advance > 0:
			...
			print (advances_ids, sum_advances)
...
class ResPartner(models.Model):
	...
	def _advance_total(self):
		...
		for price in prices:
			...
			#print( self.env['account.move'].browse(price.move_id.id).name )
			#print(price.price_total)
			if self.env['account.move'].browse(price.move_id.id).move_type == 'out_refund':
				ref_total_advance = ref_total_advance - price.price_total
			else:
				ref_total_advance = ref_total_advance + price.price_total

		self.total_advance = ref_total_advance
		#self.total_advance = sum(price.price_total for price in prices) if prices else 0
		print ('prices_2#############', self.total_advance)
