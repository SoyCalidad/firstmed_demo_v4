...
class PosReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, sales):
        ...
        sheet.merge_range(0, 0, 0, 7, 'REPORTE DE VENTA', format_title)

        sheet.write(2, 0, 'FECHA DE PEDIDO', bold_n)
        sheet.write(2, 1, 'FECHA DE FACTURA', bold_n)
        sheet.write(2, 2, 'CLIENTE', bold_n)
        sheet.write(2, 3, 'VENDEDOR', bold_n)
        sheet.write(2, 4, 'EMAIL', bold_n)
        sheet.write(2, 5, 'TELEFONO', bold_n)
        sheet.write(2, 6, 'PRODUCTOS', bold_n)
        sheet.write(2, 7, 'CANTIDAD', bold_n)
        ...
        for obj in sales:
            ...
            sheet.write(row, 3, obj.user_id.name, bold_sn)
            sheet.write(row, 4, obj.email, bold_sn)
            sheet.write(row, 5, obj.phone, bold_sn)
            for line in obj.order_line:
                sheet.write(row, 6, line.product_id.name, bold_sn)
                sheet.write(row, 7, line.product_uom_qty, bold_sn)
